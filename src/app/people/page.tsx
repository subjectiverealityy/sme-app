...
import { LuCircleCheck, LuEye, LuMessageCircle, LuPencil, LuTrash2 } from "react-icons/lu";
                  ...
                  return (
                    <tr key={c.key} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/60">
                      ...
                      <td className="px-4 py-3"><span className="flex justify-end gap-1.5">
                        <Link href={`/people/${encodeURIComponent(normalizeName(c.name))}`} title="View details" aria-label={`View ${c.name}`} className="inline-flex items-center gap-1.5 rounded-lg p-2 text-gray-500 hover:bg-[#d9f5ed] hover:text-[#272047]"><LuEye size={17} /><span className="hidden xl:inline text-[12px] font-bold">View</span></Link>
                        {c.txns[0] && <Link href={`/transactions/${c.txns[0].id}?edit=1`} title="Edit debtor record" aria-label={`Edit ${c.name}`} className="inline-flex items-center gap-1.5 rounded-lg p-2 text-gray-500 hover:bg-[#d9f5ed] hover:text-[#272047]"><LuPencil size={17} /><span className="hidden xl:inline text-[12px] font-bold">Edit</span></Link>}
                        {status === "Paid" ? <span title="Paid contacts do not need reminders" className="inline-flex cursor-not-allowed items-center gap-1.5 rounded-lg bg-gray-100 p-2 text-gray-300 blur-[1px]"><LuMessageCircle size={17} /><span className="hidden xl:inline text-[12px] font-bold">Remind</span></span> : reminder ? <a href={reminder} target="_blank" rel="noopener noreferrer" title="Send WhatsApp reminder" aria-label={`Remind ${c.name} on WhatsApp`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#d9f5ed] p-2 text-[#0b938e] hover:bg-[#bfeee2]"><LuMessageCircle size={17} /><span className="hidden xl:inline text-[12px] font-bold">Remind</span></a> : <span title="Add a phone number to remind" className="inline-flex cursor-not-allowed items-center gap-1.5 rounded-lg bg-gray-100 p-2 text-gray-300"><LuMessageCircle size={17} /><span className="hidden xl:inline text-[12px] font-bold">Remind</span></span>}
                        {status === "Credit" && <button onClick={() => markPaid(c)} disabled={paying === c.key} title="Mark all credit records as paid" aria-label={`Mark ${c.name} as paid`} className="inline-flex items-center gap-1.5 rounded-lg bg-[#29224e] p-2 text-white hover:bg-[#3b3267] disabled:opacity-50"><LuCircleCheck size={17} /><span className="hidden xl:inline text-[12px] font-bold">{paying === c.key ? "Saving" : "Paid"}</span></button>}
                        <button onClick={() => deletePerson(c)} title="Delete person" aria-label={`Delete ${c.name}`} className="inline-flex items-center gap-1.5 rounded-lg p-2 text-gray-500 hover:bg-red-50 hover:text-red-600"><LuTrash2 size={17} /><span className="hidden xl:inline text-[12px] font-bold">Delete</span></button>
                      </span></td>
                    </tr>
                  );
                })}
